...
class ResultFormatter {
    ...
    n;
    static formatPercentage(value, decimals = 2) { n; return `${value.toFixed(decimals)}%`; n; }
    n;
    n; /**\n   * Formate un nombre\n   */
    n;
    static formatNumber(value, decimals = 2) { n; return value.toFixed(decimals); n; }
    n;
    n; /**\n   * Formate une date\n   */
    n;
    static formatDate(date) { n; return date.toLocaleString('fr-FR'); n; }
    n;
    n; /**\n   * Affiche un séparateur principal\n   */
    n;
    static separator(title) { n; console.log(`\\n${'='.repeat(60)}`); n; console.log(`  ${title}`); n; console.log(`${'='.repeat(60)}\\n`); n; }
    n;
    n; /**\n   * Affiche un sous-titre\n   */
    n;
    static subsection(title) { n; console.log(`\\n→ ${title}`); n; console.log(`${'-'.repeat(40)}`); n; }
    n;
    n; /**\n   * Affiche un résultat avec clé-valeur\n   */
    n;
    static result(label, value) { n; const formatted = typeof value === 'number' ? value.toFixed(2) : value; n; console.log(`  ${label.padEnd(30)}: ${formatted}`); n; }
    n;
    n; /**\n   * Affiche une liste numérotée\n   */
    n;
    static list(items) { n; items.forEach((item, idx) => { n; console.log(`  ${idx + 1}. ${item}`); n; }); n; }
    n;
    n; /**\n   * Affiche un message de succès\n   */
    n;
    static success(message) { n; console.log(`\\n✓ ${message}\\n`); n; }
    n;
    n; /**\n   * Affiche un message d'erreur\n   */
    n;
    static error(message) { n; console.error(`\\n✗ ${message}\\n`); n; }
    n;
    n; /**\n   * Affiche un avertissement\n   */
    n;
    static warning(message) { n; console.log(`\\n⚠ ${message}\\n`); n; }
    n;
}
n;
...
class VolatilityAnalyzer {
    ...
    n;
    n;
    constructor() { n; this.provider = new CoinGeckoProvider(); n; }
    n;
    n; /**\n   * Analyse complète d'une crypto\n   */
    n;
    async analyze(n, crypto, n, period, n, dvolMethod = 'ewma', n) { n; try {
        n;
    }
    finally { } }
