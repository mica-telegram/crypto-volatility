...
async function main() {
    try {
        ...
        const period = '30d';
        const dvolMethods = ['simple', 'ewma', 'garch'];
        ...
        if (dvolResults.size > 1) {
            ...
            const dvolStd = Math.sqrt(dvolValues.reduce((sum, v) => sum + Math.pow(v - dvolAvg, 2), 0) / dvolValues.length);
